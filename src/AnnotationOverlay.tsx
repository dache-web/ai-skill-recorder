...
type Interaction = { annotationId: string; annotationType: Annotation['type']; initialFontSize: number; mode: 'move' | 'resize'; corner?: ResizeCorner; pointerId: number; startX: number; startY: number; geometry: Annotation['geometry']; historyStarted: boolean; removeWindowListeners?: () => void }
type Props = { annotations: Annotation[]; assets: RuntimeAsset[]; selectedId: string | null; onSelect: (id: string | null) => void; onInteractionStart: () => void; onGeometryChange: (id: string, geometry: Annotation['geometry'], fontSize?: number) => void; onInteractionEnd: () => void; onDelete: (id: string) => void; placementTool: PlacementTool | null; onPlace: (tool: PlacementTool, normalizedX: number, normalizedY: number) => void; onTextChange: (id: string, text: string) => void; onTextCommit: () => void; editingTextId: string | null; onMetricsChange: (metrics: OverlayMetrics) => void }
...
export default function AnnotationOverlay({ annotations, assets, selectedId, onSelect, onInteractionStart, onGeometryChange, onInteractionEnd, onDelete, placementTool, onPlace, onTextChange, onTextCommit, editingTextId, onMetricsChange }: Props) {
  ...
  const continueInteractionAt = (pointerId: number, clientX: number, clientY: number) => {
    ...
    const geometry = interaction.mode === 'move' ? moveAnnotationGeometry(interaction.geometry, deltaX, deltaY) : resizeAnnotationGeometry(interaction.geometry, interaction.corner ?? 'se', deltaX, deltaY, interaction.annotationType === 'image')
    const scale = interaction.mode === 'resize' && interaction.annotationType === 'text' ? Math.min(geometry.width / interaction.geometry.width, geometry.height / interaction.geometry.height) : 1
    const fontSize = interaction.annotationType === 'text' && interaction.mode === 'resize' ? Math.min(160, Math.max(10, interaction.initialFontSize * scale)) : undefined
    onGeometryChange(interaction.annotationId, geometry, fontSize)
  }
  const startInteraction = (event: ReactPointerEvent<HTMLElement>, annotation: Annotation, mode: 'move' | 'resize', corner?: ResizeCorner) => {
    ...
    interactionRef.current = { annotationId: annotation.id, annotationType: annotation.type, initialFontSize: annotation.style.fontSize ?? 34, mode, corner, pointerId, startX: event.clientX, startY: event.clientY, geometry: annotation.geometry, historyStarted: false, removeWindowListeners }
  ...
  return <div ref={layerRef} className={`annotation-layer ${placementTool ? 'is-placing' : ''}`} onPointerMove={(event) => continueInteractionAt(event.pointerId, event.clientX, event.clientY)} onPointerUp={(event) => finishInteraction(event.pointerId)} onPointerCancel={(event) => finishInteraction(event.pointerId)} onLostPointerCapture={(event) => finishInteraction(event.pointerId, false)} onPointerDown={(event) => {
    ...
    {annotations.map((annotation) => {
      ...
      return <div key={annotation.id} className={`annotation-object ${selected ? 'selected' : ''}`} style={{ left: `${annotation.geometry.x * 100}%`, top: `${annotation.geometry.y * 100}%`, width: `${annotation.geometry.width * 100}%`, height: `${annotation.geometry.height * 100}%`, transform: `rotate(${annotation.geometry.rotationDegrees}deg)`, opacity: annotation.style.opacity }}>
        <div role="button" tabIndex={0} className={`annotation annotation-${annotation.type}`} aria-label={`${annotation.type}注釈を選択・移動`} onPointerDown={(event) => startInteraction(event, annotation, 'move')} style={{ color: annotation.style.textColor, borderColor: annotation.style.strokeColor, fontSize: annotation.type === 'text' ? `${annotation.style.fontSize ?? 34}px` : undefined }}>{asset ? <img src={asset.previewUrl} alt="重ね画像" draggable={false} /> : annotationLabel(annotation)}</div>
        ...
        {selected && <><span className="selection-frame" aria-hidden="true" />{editingTextId !== annotation.id && (['nw', 'ne', 'sw', 'se'] as const).map((corner) => <button key={corner} type="button" className={`resize-handle handle-${corner}`} aria-label={`${corner}ハンドルでサイズ変更`} onPointerDown={(event) => startInteraction(event, annotation, 'resize', corner)} />)}{editingTextId !== annotation.id && <button type="button" className="annotation-delete" aria-label="選択中の注釈を削除" onClick={(event) => { event.stopPropagation(); onDelete(annotation.id) }}>×</button>}</>}
      </div>
    })}
  </div>
}
