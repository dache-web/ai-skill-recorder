...
function App() {
  ...
  const addAnnotation = (type: AnnotationType, text = '') => {
    ...
    const start = selectedSegment ? normalizeTime(previewRef.current?.currentTime ?? selectedSegment.startSeconds) : null
    if (start !== null) setReviewCurrentTime(start)
    const annotation: Annotation = { id: `annotation-${sequence}`, targetTimelineId: selectedTimelineItem.id, type, source: 'user', status: 'accepted', startSeconds: start, endSeconds: selectedSegment ? selectedSegment.endSeconds : null, geometry: { x: .38, y: .38, width: .24, height: .16, rotationDegrees: 0 }, style: { strokeColor: '#e13f2b', fillColor: type === 'rectangle' || type === 'ellipse' ? 'rgba(255,255,255,0)' : '#e13f2b', textColor: '#172525', strokeWidth: 4, opacity: 1, fontSize: 34 }, text }
  ...
  const updateAnnotationGeometry = (key: keyof Annotation['geometry'], value: number) => {
    const target = annotations.find((annotation) => annotation.id === selectedAnnotationId); if (!target) return
    updateSelectedAnnotation({ geometry: { ...target.geometry, [key]: value } })
  }
  const deleteSelectedAnnotation = () => { if (!selectedAnnotationId) return; recordEdit(); setAnnotations((current) => current.filter((annotation) => annotation.id !== selectedAnnotationId)); setSelectedAnnotationId(null); markTimelineChanged(); setReviewMessage('注釈を削除しました。') }
  ...
  const importImage = async (file: File | null) => {
    ...
    try {
      ...
      if (imageInsertModeRef.current === 'overlay') {
        ...
        setAnnotations((current) => [...current, { id: annotationId, targetTimelineId: selectedTimelineItem.id, type: 'image', source: 'user', status: 'accepted', startSeconds: selectedSegment ? normalizeTime(previewRef.current?.currentTime ?? selectedSegment.startSeconds) : null, endSeconds: selectedSegment?.endSeconds ?? null, geometry: { x: .62, y: .62, width: .3, height: .28, rotationDegrees: 0 }, style: { strokeColor: '#ffffff', fillColor: '#ffffff', textColor: '#172525', strokeWidth: 0, opacity: 1 }, assetId: asset.data.id }]); setSelectedAnnotationId(annotationId); markTimelineChanged()
      } else {
  ...
  const timelineCard = (item: TimelineItem, index: number) => {
    ...
    return <article ref={(element) => { timelineCardRefs.current[item.id] = element }} key={item.id} onClick={() => selectTimelineItem(item)} onDragOver={(event) => event.preventDefault()} onDrop={() => dropTimelineItem(item.id)} className={`timeline-item compact ${item.status === 'excluded' ? 'is-excluded' : ''} ${isSelected ? 'is-reviewing' : ''}`} aria-current={isSelected ? 'true' : undefined}>
      <div className="timeline-card-heading"><span className="drag-handle" draggable onDragStart={(event) => { event.stopPropagation(); setDraggedTimelineId(item.id) }} onDragEnd={() => setDraggedTimelineId(null)} title="ドラッグして順番を変更">☰</span><strong>{index + 1}. {label}</strong></div>
      ...
      <section className="review-card" aria-labelledby="review-title">
        ...
        {reviewUrl ? <div ref={reviewWorkspaceRef} className={`review-workspace redesigned mode-${workspaceMode}`}>
          <div className="review-stage" hidden={workspaceMode !== 'edit'}>
            {videoSourceControls}
            <div className="media-editor"><div className="media-viewport">
              ...
              {(selectedPoint || selectedSlideAsset) && <img ref={mainImageRef} className="main-static-image" src={selectedPoint?.previewUrl ?? selectedSlideAsset?.previewUrl} alt="現在編集中の静止画" />}
              ...
              <div className="annotation-layer">{currentAnnotations.map((annotation) => { const asset = annotation.assetId ? insertedAssets.find((candidate) => candidate.data.id === annotation.assetId) : null; return <button type="button" key={annotation.id} className={`annotation annotation-${annotation.type} ${selectedAnnotationId === annotation.id ? 'selected' : ''}`} onClick={() => setSelectedAnnotationId(annotation.id)} style={{ left: `${annotation.geometry.x * 100}%`, top: `${annotation.geometry.y * 100}%`, width: `${annotation.geometry.width * 100}%`, height: `${annotation.geometry.height * 100}%`, transform: `rotate(${annotation.geometry.rotationDegrees}deg)`, opacity: annotation.style.opacity, color: annotation.style.textColor, borderColor: annotation.style.strokeColor }}>{asset ? <img src={asset.previewUrl} alt="重ね画像" /> : annotation.type === 'ellipse' ? '○' : annotation.type === 'rectangle' ? '□' : annotation.type === 'arrow' ? '➜' : annotation.type === 'line' ? '―' : annotation.text || (annotation.type === 'check' ? '✓' : annotation.type === 'warning' ? '⚠' : '1')}</button> })}</div>
            </div><aside className="annotation-toolbar" aria-label="注釈ツール"><details><summary>記号</summary><div className="shape-menu"><button type="button" onClick={() => addAnnotation('arrow')}>矢印</button><button type="button" onClick={() => addAnnotation('ellipse')}>○</button><button type="button" onClick={() => addAnnotation('rectangle')}>□</button><button type="button" onClick={() => addAnnotation('line')}>線</button><button type="button" onClick={() => addAnnotation('callout', '説明')}>吹出</button><button type="button" onClick={() => addAnnotation('step-number', '1')}>番号</button><button type="button" onClick={() => addAnnotation('check')}>✓</button><button type="button" onClick={() => addAnnotation('warning')}>注意</button></div></details><button type="button" onClick={() => addAnnotation('text', 'テキスト')}>T</button><details><summary>画像</summary><button type="button" onClick={() => { imageInsertModeRef.current = 'overlay'; imageInputRef.current?.click() }}>画面へ重ねる</button><button type="button" onClick={() => { imageInsertModeRef.current = 'slide'; imageInputRef.current?.click() }}>ページで追加</button></details><hr /><button type="button" onClick={() => void saveCurrentView()}>画像を保存</button><input ref={imageInputRef} hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => void importImage(event.target.files?.[0] ?? null)} /></aside></div>
            ...
            <div className="timeline-toolbar"><button type="button" onClick={() => moveTimelineSelection(-1)} disabled={!visibleTimelineItems.length}>← 前へ</button><strong>{selectedTimelineId ? visibleTimelineItems.findIndex((item) => item.id === selectedTimelineId) + 1 : 0} / {visibleTimelineItems.length}</strong><button type="button" onClick={() => moveTimelineSelection(1)} disabled={!visibleTimelineItems.length}>次へ →</button><button type="button" onClick={() => setWorkspaceMode('organize')}>一覧で整理</button><button type="button" onClick={() => { setShowTrash(true); setWorkspaceMode('organize') }}>ゴミ箱</button><button type="button" onClick={undoEdit} disabled={!pastRef.current.length}>戻る</button><button type="button" onClick={redoEdit} disabled={!futureRef.current.length}>進む</button></div>
            ...
            {selectedAnnotationId && (() => { const annotation = annotations.find((candidate) => candidate.id === selectedAnnotationId); return annotation ? <div className="annotation-editor"><strong>注釈を編集</strong>{annotation.type === 'text' && <input value={annotation.text ?? ''} onChange={(event) => updateSelectedAnnotation({ text: event.target.value })} />}<label>X<input type="range" min="0" max="0.9" step="0.01" value={annotation.geometry.x} onChange={(event) => updateAnnotationGeometry('x', Number(event.target.value))} /></label><label>Y<input type="range" min="0" max="0.9" step="0.01" value={annotation.geometry.y} onChange={(event) => updateAnnotationGeometry('y', Number(event.target.value))} /></label><label>幅<input type="range" min="0.05" max="0.8" step="0.01" value={annotation.geometry.width} onChange={(event) => updateAnnotationGeometry('width', Number(event.target.value))} /></label><label>高さ<input type="range" min="0.05" max="0.8" step="0.01" value={annotation.geometry.height} onChange={(event) => updateAnnotationGeometry('height', Number(event.target.value))} /></label><label>回転<input type="range" min="-180" max="180" value={annotation.geometry.rotationDegrees} onChange={(event) => updateAnnotationGeometry('rotationDegrees', Number(event.target.value))} /></label>{annotation.startSeconds !== null && <><button type="button" onClick={() => updateSelectedAnnotation({ startSeconds: currentVideoTime() ?? annotation.startSeconds })}>表示開始を現在位置</button><button type="button" onClick={() => updateSelectedAnnotation({ endSeconds: currentVideoTime() ?? annotation.endSeconds })}>表示終了を現在位置</button></>}<button className="danger-link" type="button" onClick={deleteSelectedAnnotation}>注釈を削除</button></div> : null })()}
            {fullReviewActive && <p className="timeline-notice">全体確認中です。固定5ボタンと各カード操作で、そのまま修正できます。</p>}
            {visibleTimelineItems.length === 0 ? <p className="empty-state">動画を見ながら、動画区間・★ポイント・削除予定を登録してください。</p> : <div ref={timelineStripRef} className="manual-timeline horizontal">{visibleTimelineItems.map(timelineCard)}</div>}
