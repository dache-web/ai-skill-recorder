...
- 録画停止後のアプリ内再生
- 利用者が選んだPC内の場所への保存
- 録画直後または保存済みWebMから解析用の基本情報と時刻付きPNGを生成
- 元WebM・解析用JSON・PNGを人が確認してPCへ保存

STEP2-1の解析準備はすべてブラウザ内で行います。AI API、文字起こし、外部アップロードはまだ使用しません。静止画は2秒・5秒・10秒間隔から選択でき、最大30枚に制限されます。
